...
def find_ffmpeg():
    ...
    # version
    try:
        output = subprocess.check_output(binary + b' -version', shell=True)

    except subprocess.CalledProcessError as e:
        logging.error('ffmpeg: could find version: %s' % e)
        return None, None, None

    result = re.findall(b'ffmpeg version (.+?) ', output, re.IGNORECASE)
    version = result and result[0] or ''

    # codecs
    try:
        output = subprocess.check_output(binary + b' -codecs -hide_banner', shell=True)
    ...
    lines = [l for l in lines if re.match(b'^ [DEVILSA.]{6} [^=].*', l)]

    codecs = {}
    for line in lines:
        m = re.match(b'^ [DEVILSA.]{6} ([\w+_]+)', line)
        if not m:
            continue
        ...
        m = re.search(b'decoders: ([\w\s_]+)+', line)
        if m:
            decoders = set(m.group(1).split())

        m = re.search(b'encoders: ([\w\s_]+)+', line)
        if m:
...
def cleanup_media(media_type):
    ...
    for camera_id in config.get_camera_ids():
        ...
        if os.path.exists(target_dir):
            ...
            open(os.path.join(target_dir, '.keep'), 'wb').close()
...
def make_movie_preview(camera_config, full_path):
    ...
    cmd = b'ffmpeg -i %(path)s -f mjpeg -vframes 1 -ss %(offs)s -y %(path)s.thumb'
...
def make_timelapse_movie(camera_config, framerate, interval, group):
    ...
    def make_movie(pictures):
        ...
        cmd = b'rm -f %(tmp_filename)s;'
        cmd += b'cat %(jpegs)s | ffmpeg -framerate %(framerate)s -f image2pipe -vcodec mjpeg -i - -vcodec %(codec)s ' \
               b'-format %(format)s -b:v %(bitrate)s -qscale:v 0.1 -f avi %(tmp_filename)s'
    ...
    def poll_movie_process(pictures):
        ...
        if _timelapse_process.poll() is None:  # not finished yet
            ...
            output = _timelapse_process.stdout.read() or b''

            frame_index = re.findall(b'frame=\s*(\d+)', output)
            try:
